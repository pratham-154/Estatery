...
const getListing = async (req, select = {}, where = {}, joins = []) => {
    try {
        ...
        let listing = await response.find(where, select, { skip: offset })
            .populate(joins)
            .sort(orderBy)
            .limit(limit)
            .exec();

        let totalCount = await response.countDocuments(where);
...
const getAll = async(where ={} ,select={} , joins =[] , orderBy= {'title':1} , limit = 10 )=>{
    try {
        let query =  response.find(where, select);
...
const get = async (id, select = [], joins = []) => {
    try
    {   
        let record = response.findById(id,select);
...
const getRow = async (where,select = []) => {
    try
    {
        let record = await response.findOne(where, select).exec();
        return record;
    }
    catch(error)
    {
...
const modify = async (id, data) => {
    try
    {
        ...
        let resp = await response.updateOne(
            {
                "_id":id
            },
            data
...
const modifyAll = async (ids, data) => {
    try
    {
        let resp = await response.updateMany(
            {
                '_id':{
                    $in:ids
                }
...
const remove = async (id) => {
    try
    {
        ...
        if(getRecord)
        {
            let record = await response.deleteOne({
                '_id':id
            }).exec();
...
const removeAll = async (ids) => {
    try
    {
        let record = await response.deleteMany(
            {
                '_id':{
                    $in:ids
                }
...
const getCounts = async (where = {}) => {
    try
    {
        let record = await response.countDocuments(where).exec();
        return record;
    }
    catch(error)
    {
