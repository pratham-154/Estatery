...
const getListing = async (req, select = {}, where = {}, joins = []) => {
    try
    {
        ...
        let listing = await feature.find(where, select, {skip:offset})
            .populate(joins)
            .sort(orderBy)
            .limit(limit);
...
const getAll = async(where ={} ,select={} , joins =[] , orderBy= {'title':1} , limit = 10 )=>{
    try {
        let query =  feature.find(where, select);
...
const get = async (id, select = [], joins = []) => {
    try
    {   
        let record = feature.findById(id,select);
...
const getRow = async (where,select = []) => {
    try
    {
        let record = await feature.findOne(where, select).exec();
        return record;
    }
    catch(error)
    {
...
const modify = async (id, data) => {
    try
    {
        ...
        let resp = await feature.updateOne(
            {
                "_id":id
            },
            data
...
const modifyAll = async (ids, data) => {
    try
    {
        let resp = await feature.updateMany(
            {
                '_id':{
                    $in:ids
                }
...
const remove = async (id) => {
    try
    {
        ...
        if(getRecord)
        {
            let record = await feature.deleteOne({
                '_id':id
            }).exec();
...
const removeAll = async (ids) => {
    try
    {
        let record = await feature.deleteMany(
            {
                '_id':{
                    $in:ids
                }
...
const getCounts = async (where = {}) => {
    try
    {
        let record = await feature.countDocuments(where).exec();
        return record;
    }
    catch(error)
    {
