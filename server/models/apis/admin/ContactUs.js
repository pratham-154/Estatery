...
const getListing = async (req, select = {}, where = {}, joins = []) => {
    try
    {
        ...
        let listing = await contact.find(where, select, {skip:offset})
            .populate(joins)
            .sort(orderBy)
            .limit(limit);
...
const getAll = async(where ={} ,select={} , joins =[] , orderBy= {'title':1} , limit = 10 )=>{
    try {
        let query =  contact.find(where, select);
...
const get = async (id, select = [], joins = []) => {
    try
    {   
        let record = contact.findById(id,select);
...
const getRow = async (where,select = []) => {
    try
    {
        let record = await contact.findOne(where, select).exec();
        return record;
    }
    catch(error)
    {
...
const modify = async (id, data) => {
    try
    {
        ...
        let resp = await contact.updateOne(
            {
                "_id":id
            },
            data
...
const modifyAll = async (ids, data) => {
    try
    {
        let resp = await contact.updateMany(
            {
                '_id':{
                    $in:ids
                }
...
const remove = async (id) => {
    try
    {
        ...
        if(getRecord)
        {
            let record = await contact.deleteOne({
                '_id':id
            }).exec();
...
const removeAll = async (ids) => {
    try
    {
        let record = await contact.deleteMany(
            {
                '_id':{
                    $in:ids
                }
