...
const getAll = async(where ={} ,select={} , joins =[] , orderBy= {'title':1} , limit = 10 )=>{
    try {
        let query =  about.find(where, select);
...
const getRow = async (where,select = []) => {
    try
    {
        let record = await about.findOne(where, select).exec();
        return record;
    }
    catch(error)
    {
...
const modify = async (id, data) => {
    try
    {
        ...
        let resp = await about.updateOne(
            {
                "_id":id
            },
            data
...
const modifyAll = async (ids, data) => {
    try
    {
        let resp = await about.updateMany(
            {
                '_id':{
                    $in:ids
                }
...
const remove = async (id) => {
    try
    {
        ...
        if(getRecord)
        {
            let record = await about.deleteOne({
                '_id':id
            }).exec();
...
const removeAll = async (ids) => {
    try
    {
        let record = await about.deleteMany(
            {
                '_id':{
                    $in:ids
                }
...
const getCounts = async (where = {}) => {
    try
    {
        let record = await about.countDocuments(where).exec();
        return record;
    }
    catch(error)
    {
