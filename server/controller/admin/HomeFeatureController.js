const featureModel  =  require("../../models/apis/admin/HomeFeature");
...
const index = async (req, res) => {
    ...
    let data = await featureModel.getListing(req, select, where);
...
const add = async (req, res) => {
    ...
    {
        let resp = await featureModel.create(data);
...
const edit = async (req, res) => {
    ...
    if(!validatorRules.fails())
    {
        let resp = await featureModel.modify(id, data);
        if(resp)
        {
            res.send({
                'status':true,
...
const remove = async (req, res) => {
    ...
    let resp = await featureModel.remove(id);
...
const view = async (req, res) => {
    let {id} = req.params;
    let resp = await featureModel.get(id);    
    if(resp)
    {
        res.send({
            'status':true,
...
const bulkAction = async (req, res) => {
    ...
    if(ids && ids.length > 0 && type)
    {
        ...
        {
            case 'active':
                await featureModel.modifyAll(ids, {
                    'status': 1
                });
            ...
            break;
            case 'delete':
                await featureModel.removeAll(ids);
