const contactModel  =  require("../../models/apis/admin/ContactUs");
...
const edit = async (req, res) => {
    ...
    if(!validatorRules.fails())
    {
        let resp = await contactModel.modify(id, data);
        if(resp)
        {
            res.send({
                'status':true,
...
const remove = async (req, res) => {
    ...
    let resp = await contactModel.remove(id);
...
const view = async (req, res) => {
    let {id} = req.params;
    let resp = await contactModel.get(id);    
    if(resp)
    {
        res.send({
            'status':true,
...
const bulkAction = async (req, res) => {
    ...
    if(ids && ids.length > 0 && type)
    {
        ...
        {
            case 'active':
                await contactModel.modifyAll(ids, {
                    'status': 1
                });
            ...
            break;
            case 'delete':
                await contactModel.removeAll(ids);
