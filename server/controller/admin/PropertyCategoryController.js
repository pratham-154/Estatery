const categoryModel  =  require("../../models/apis/admin/PropertyCategory");
...
const index = async (req, res) => {
    ...
    let data = await categoryModel.getListing(req, select, where);
...
const add = async (req, res) => {
    ...
    {
        let resp = await categoryModel.create(data);
...
const edit = async (req, res) => {
    ...
    if(!validatorRules.fails())
    {
        let resp = await categoryModel.modify(id, data);
        if(resp)
        {
            res.send({
                'status':true,
...
const remove = async (req, res) => {
    ...
    let resp = await categoryModel.remove(id);
...
const view = async (req, res) => {
    let {id} = req.params;
    let resp = await categoryModel.get(id);    
    if(resp)
    {
        res.send({
            'status':true,
...
const bulkAction = async (req, res) => {
    ...
    if(ids && ids.length > 0 && type)
    {
        ...
        {
            case 'active':
                await categoryModel.modifyAll(ids, {
                    'status': 1
                });
            ...
            break;
            case 'delete':
                await categoryModel.removeAll(ids);
