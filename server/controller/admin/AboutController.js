const aboutModel  =  require("../../models/apis/admin/About");
...
const add = async (req, res) => {
    ...
    if(!validatorRules.fails())
    {
        ...
        let resp = await aboutModel.create(data);
        if(resp)
        {
            res.send({
                'status':true,
...
const edit = async (req, res) => {
    ...
    if(!validatorRules.fails())
    {
        let resp = await aboutModel.modify(id, data);
        if(resp)
        {
            res.send({
                'status':true,
...
const remove = async (req, res) => {
    ...
    let resp = await aboutModel.remove(id);
...
const bulkAction = async (req, res) => {
    ...
    if(ids && ids.length > 0 && type)
    {
        ...
        {
            case 'active':
                await aboutModel.modifyAll(ids, {
                    'status': 1
                });
            ...
            break;
            case 'delete':
                await aboutModel.removeAll(ids);
